...
function processTick(previous: GameState): GameState {
  ...
  if (settlesPower) {
    ...
    else state.message = `전력 정산 완료 · ${state.lastPowerDelta >= 0 ? "+" : ""}${state.lastPowerDelta} · 생산 틱 ${state.tick}`;
  } else state.message = `생산 틱 ${state.tick} 완료 · 전력 정산까지 ${POWER_SETTLEMENT_TICKS - state.tick % POWER_SETTLEMENT_TICKS}틱`;
  return state;
}
